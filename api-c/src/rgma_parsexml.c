 ...
 *
 * @param  buf      buffer to parse (will be modified by this parser)
 * @param  bufsize  number of bytes in buffer
 *
 * @return Head of element tree, or NULL on memory/parse error.
 */

PUBLIC ELEMENT *parsexml_parse(char *buf, int bufsize) {
    ELEMENT *e, *root;
    char *token, *s, *t, *tag_name;
    int i, state, end_tag, quote_character, last_c;
    ...
    for (i = 0; i < bufsize; ++i) {
        ...
        case STATE_PI: /* Inside <? .... ?> before main document */
...
PRIVATE ELEMENT *start_element(ELEMENT *e, char *name, char *terminator) {
    ...
    *terminator = '\0';
    ...
    if (e == NULL) {
        ...
        e->children = new;
        if (e->data)
            e->data[0] = '\0'; /* scrub any element data */
        new->parent = e;
    } else {
        e->next = new;
        new->parent = e->parent;
...
PRIVATE ELEMENT *end_element(ELEMENT *e, char *name, char *terminator) {
    ...
    if (e == NULL) {
        return NULL; /* end tag without start tag */
    }
    e->closed = TRUE;
...
PRIVATE ELEMENT *add_attribute(ELEMENT *e, char *key, char *terminator) {
    ...
    *terminator = '\0';
    ...
    if (e->atts == NULL) {
        ...
        for (a = e->atts; a->next != NULL; a = a->next)
            ; /* last one */
        a->next = new;
    }
 ...
 *
 * @return NULL always (for convenience).
 */
...
PRIVATE void *free_elements(ELEMENT *root) {
    ...
    for (e = root; e != NULL; e = next_e) {
        ...
        if (e->children) {
            free_elements(e->children); /* recursive */
        }
        next_e = e->next;
        free(e);
    }
    return NULL; /* just a convenience */
}
...
PRIVATE void decode_entities(char *buf) {
    ...
    for (s = buf; *s != '\0'; ++s) {
        if (s[0] == '&' && s[1] == 'a' && s[2] == 'm' && s[3] == 'p' && s[4] == ';') {
            *t++ = '&';
            s += 4;
        } else if (s[0] == '&' && s[1] == 'a' && s[2] == 'p' && s[3] == 'o' && s[4] == 's' && s[5] == ';') {
            *t++ = '\'';
            s += 5;
        } else if (s[0] == '&' && s[1] == 'g' && s[2] == 't' && s[3] == ';') {
            *t++ = '>';
            s += 3;
        } else if (s[0] == '&' && s[1] == 'l' && s[2] == 't' && s[3] == ';') {
            *t++ = '<';
            s += 3;
        } else if (s[0] == '&' && s[1] == 'q' && s[2] == 'u' && s[3] == 'o' && s[4] == 't' && s[5] == ';') {
            *t++ = '"';
            s += 5;
        } else
            *t++ = *s;
